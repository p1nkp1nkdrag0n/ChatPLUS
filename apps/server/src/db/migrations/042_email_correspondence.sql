...
-- Permit instant in-app email while preserving physical-letter deadlines,
-- rowids, encrypted replies, foreign keys, indexes and durability triggers.
CREATE TEMP TABLE letters_email_backup AS SELECT rowid AS legacy_rowid, * FROM letters;
DROP TABLE letters;
...
CREATE TABLE IF NOT EXISTS letters (
  ...
  delivery_method TEXT NOT NULL DEFAULT 'standard'
    CHECK (delivery_method IN ('standard', 'express', 'priority', 'email')),
  subject TEXT,
  body TEXT,
  ...
  transit_policy_version TEXT CHECK (
    ...
    OR transit_policy_version IN ('fixed_5d_v1', 'fixed_2d_v1', 'fixed_1d_v1', 'fixed_0d_v1')
  ),
  ...
  CHECK (transit_policy_version IS NULL OR
    ...
    (delivery_method = 'priority' AND transit_policy_version = 'fixed_1d_v1') OR
    (delivery_method = 'email' AND transit_policy_version = 'fixed_0d_v1')),
  CHECK (
  ...
  CHECK (
    ...
    (status IN ('sealed', 'in_transit', 'delivered_unread', 'read')
      ...
      AND ((delivery_method = 'email' AND dispatched_at_utc = arrival_due_at_utc)
        OR (delivery_method <> 'email' AND dispatched_at_utc < arrival_due_at_utc)))
  ),
  CHECK (
...
INSERT INTO letters (rowid, id, thread_id, agent_id, create_request_id, create_request_hash, seal_request_id, reply_to_letter_id, direction, status, delivery_method, subject, body, content_hash, encrypted_ciphertext, encrypted_iv, encrypted_auth_tag, encrypted_key_version, encrypted_aad_hash, encrypted_created_at_utc, transit_policy_version, transit_timezone, dispatched_at_utc, arrival_due_at_utc, effective_author_time_utc, delivered_effective_at_utc, processed_at_utc, read_at_utc, opened_at_utc, created_at_utc, updated_at_utc)
SELECT legacy_rowid, id, thread_id, agent_id, create_request_id, create_request_hash, seal_request_id, reply_to_letter_id, direction, status, delivery_method, subject, body, content_hash, encrypted_ciphertext, encrypted_iv, encrypted_auth_tag, encrypted_key_version, encrypted_aad_hash, encrypted_created_at_utc, transit_policy_version, transit_timezone, dispatched_at_utc, arrival_due_at_utc, effective_author_time_utc, delivered_effective_at_utc, processed_at_utc, read_at_utc, opened_at_utc, created_at_utc, updated_at_utc FROM letters_email_backup;
DROP TABLE letters_email_backup;
