...
export function interviewSubject(
  ...
  return name && name.length <= 24 ? name : "这个人";
}
...
const incompleteAnswersSchema = z
  .object({
    gender: z.string().max(CHARACTER_INTERVIEW_ANSWER_MAX_LENGTH).default(""),
    name: z.string().max(CHARACTER_INTERVIEW_ANSWER_MAX_LENGTH).default(""),
    ageText: z.string().max(CHARACTER_INTERVIEW_ANSWER_MAX_LENGTH).default(""),
      ...
      .array(
        z
          .object({
            ...
            answer: z.string().max(CHARACTER_INTERVIEW_ANSWER_MAX_LENGTH),
          })
          .strict(),
      )
      .max(2)
...
const storedDraftSchema = z
  .object({
    ...
    // JSON escaping can expand each character sixfold; preserve full answers.
    followUpSnapshot: z.string().max(200_000).optional(),
...
export function readInterviewDraft(): InterviewDraft | undefined {
  try {
    ...
    if (!raw || raw.length > 500_000) return undefined;
