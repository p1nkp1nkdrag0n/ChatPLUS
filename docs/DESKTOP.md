...
| 项目 | 普通桌面版 | 联网桌面版 |
| --- | --- | --- |
| 版本来源 | `apps/desktop/package.json` | `apps/desktop-online/package.json` |
| 安装包名称 | `Dearvale-<version>-Setup.exe` | `Dearvale-Online-<version>-Setup.exe` |
| 安装标识 | `com.dearvale.desktop` | `com.dearvale.online` |
| 程序名称 | `Dearvale.exe` | `Dearvale Online.exe` |
| 默认用户数据目录 | `%APPDATA%\Dearvale` | `%APPDATA%\Dearvale Online` |
| 功能来源 | 包内前端与本地服务器 | 所连接服务器的当前版本 |

两种客户端独立编号，项目 Git 标签也不等同于客户端安装包版本。两种客户端使用不同安装标识，可以分别安装；不要手动选择同一安装目录或共用用户数据目录。同一种客户端的多个版本共享安装标识，按升级替换管理。

构建或下载新安装包不会更新已经安装的程序。核对实际安装版本时，应查看 Windows 已安装应用中的对应产品及程序版本，不能仅看下载目录里的文件名。

分发安装包时同时提供 SHA-256，并记录对应 Git 提交和构建参数。文件名和版本号相同不能证明安装包内容相同；已经分发的版本若需要改变内容，应递增该客户端版本号，保留旧包及其校验值。安装包与校验附件通过 Releases 分发，源码仓库不保存这些构建产物。
