...
输入协议 v2 给公开消息标明发言者、来源 ID、写作时间和首次可见时间，并提供上海当地日期；书信只使用公开 API 提供的日期，不读取隐藏的写作时间。重复打开同一封信只检查产品幂等性，不再重复添加信的正文。发送前检查明确的角色错位（例如模拟用户称呼对方为自己的名字）及盲测问题 schema；遇到这些错误最多重新生成一次，仍不合格便停止，在发送到产品之前保留失败证据。`user-input-checks.jsonl` 记录每次拒绝和接受；传输错误不在此层额外重试。这些规则不能自动证明所有叙述都真实、所有角色归属都正确，仍需要监督者审阅。

`acceptance-status.json` 分别记录执行状态、功能检查、42 个场景的待审状态、语义质量待审和 README 目标尚未评价。功能检查失败不会被标成通过，跑完 42 轮也不会自动给场景和内容打合格分。恢复还要求输入协议及连接配置一致；v1 的旧实测保留用于对照，不能用 v2 接着改写同一实验。
