...
成就墙在桌面显示六档便签布局，手机使用两列；纸色、火漆跨边位置、信息层次和筛选布局应按概念图维护。实际浏览器截图与验收记录保存在本地忽略目录。

维护检查覆盖筛选、详情、键盘操作、透明火漆完整显示、两列手机布局和图片加载回退；配合类型检查、生产构建，以及组件、Provider、任务、迁移、历史版本和备份相关测试。测试成绩、截图和控制台记录保存在本地忽略目录。
