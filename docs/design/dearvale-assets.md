...
本批素材是通过内置 `image_gen.imagegen` 编辑 Dearvale 的 AI 视觉稿得到的生产插画，保留工具的原生 PNG 输出。各资产的用途、参数、文件摘要和生成提示词记录如下。
...
这些资源为用户已授权的 Dearvale 产品改版工作生成和编辑；其来源为Dearvale 视觉稿，没有从 Linear 或 Lusion 网站下载并打包其图片。Linear 与 Lusion 是页面排版、艺术氛围的参考，并非本目录内资源的文件来源。
