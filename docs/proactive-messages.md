...
生成输入包含角色时区、带偏移的当地当前时间及星期、有效联系窗口、角色人格、来源材料/跟进目标与最近 12 条对话。生成提示分别处理生活分享、明确提醒、预约联系和事项关心，预约是提醒/约定类中的独立子意图。

事项的 `grounding_json.currentSchedule` 保存安排时区、原话时间锚点、当地日期与精度；明确钟点才保存 `localTime` 和 `atUtc`，笼统上午/晚上只保存时段。该安排时间在事后关心延迟之前取得，不通过“联系窗口减两小时”倒推，也不生成事件结束时间。多次改期沿用同一事项 ID，每次保存最新改期消息对应的安排。

投递时同时提供当前角色时区和安排当时采用的时区。`latestArrangement` 使用最新来源消息，旧来源标为历史，联系目的中的旧引文不再作为生效时间；联系窗口仍单独表示允许评估的时间。旧记录若没有安排元数据，就保留来源原话与可用的时间锚点，不补造精确事件时刻。角色切换时区不会重新解释已保存的安排。

模型输出 `decision=send` 或 `decision=skip`。`skip` 是正常完成的联系决定，记录到 `proactive_intent_decisions`；同一来源版本不会一轮轮强迫模型最终发出消息。模型/网络故障返回失败，由持久化任务退避，不会以通用草稿强制发送。
