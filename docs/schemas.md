...
Legacy schedule-purpose and `ScheduleItem` contracts may remain temporarily readable for database/history compatibility, but they do not define the current product surface and new ordinary life facts must not be written through them. New code should use the explicit `LegacyExactScheduleItem` / `LegacyExactSchedulePolicy` aliases when it must touch that compatibility model. Runtime capability responses expose mutually exclusive `fuzzyLife` and `legacyExactSchedule`; the old `schedule` boolean remains only as a compatibility alias for `legacyExactSchedule`.

## API transport contracts

`packages/contracts/src/api.ts` defines the shared success-response schemas for health, character lists and publication, agent activation, sessions and messages, timeline and memory projections, settings, and SSE event variants. `FuzzyLifePromptContextSchema` is the single strict contract for the persisted fuzzy-life view used by the service, API and web UI; it preserves separate decision, action, outcome and reflection evidence instead of allowing a hand-written partial projection. Canonical timeline entries also carry a required `provenance` (`conversation`, `life_simulation`, `character_spec` or `system`) independently from the retired exact-schedule `source` field, so the ordinary UI can explain where a change came from without reviving schedule semantics. The web product exposes fuzzy life summaries and causal history, not a character schedule rail or future-72-hours calendar. The retained schedule endpoint labels every response with `dataModel: fuzzy_life | legacy_exact_schedule`; in fuzzy mode it is a retired compatibility projection and returns no exact items.
